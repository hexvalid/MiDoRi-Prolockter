...
public class APILockter {
    ...
    public static void tweetAt(Twitter hesap, String tweet) {
        try {
            ...
            else if (e.toString().contains("this account is temporarily locked.")) {
                Log.yaz("Hesap banlanmış!", HATA);
                try {
                    Veritabani.hesapSil(hesap.getScreenName());
                } catch (Exception e1) {
                    Log.yaz("Banlanmış hesabın kullanıcı adı bile alınamadı.", HATA);
                }
            } else {
